...
Installing Venndiagram for plotting venn diagram.
```{r echo = T, results = 'hide', warning=FALSE, message=FALSE}
...
##Venn diagram
```{r}
# Example vectors of gene names (replace these with your actual gene lists)
lesion_genes <- rownames(res[res$padj < 0.05 & res$tissue == "Lesion_treatment", ])
nonlesion_genes <- rownames(res[res$padj < 0.05 & res$tissue == "Nonlesion_treatment", ])
healthy_genes <- rownames(res[res$padj < 0.05 & res$tissue == "Healthy_treatment", ])


library(VennDiagram)

# Create a list of the gene sets
gene_list <- list(Lesion = lesion_genes, 
                  Nonlesion = nonlesion_genes, 
                  Healthy = healthy_genes)

# Create the Venn diagram
venn.plot <- venn.diagram(
  x = gene_list,
  category.names = c("Lesion", "Nonlesion", "Healthy"),
  ...
  cat.cex = 1.5,
  cat.pos = 0, # adjust to change category position
