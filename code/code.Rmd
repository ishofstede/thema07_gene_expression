...
Installing PoiClaClu for multidimensional scaling.
```{r echo = T, results = 'hide', warning=FALSE, message=FALSE}
if (!require(PoiClaClu)){
  install.packages("lPoiClaClu", dependencies = TRUE)
}
```
...
my_data <- read.table('../data/GSE47944_ps_tt_gel_raw.txt', header = TRUE, 
                      row.names = 1)
classification <- getGEO(filename='../data/GSE47944_series_matrix.txt')
...
EnhancedVolcano(res,
                ...
                pCutoff = 0.05,         #cutoff for p-value
                FCcutoff = 2,           #cutoff for fold change
                pointSize = 2.0,        #size of points
                labSize = 3.0,          #size of labels
                axisLabSize = 10,       #axis label size
                titleLabSize = 12,      #title label size
                subtitleLabSize = 8,    #subtitle label size
                captionLabSize = 10,     #caption label size
                legendPosition = "right"  #position of legend
)

```
X-axis tells you how much the gene is expressed (up or down), and Y-axis shows how significant this difference is. There are a lot of genes with high fold change and are therefore more up or downregulated. The downregulation is more significant than the upregulat MMP1 and CXCL% stand out the most as the most significantly downregulated genes.

```{r}
library(dplyr)

# Filter the DEGs based on adjusted p-value and log2 fold change
significant_degs <- res %>%
  as.data.frame() %>%  
  filter(padj < 0.05 & abs(log2FoldChange) >= 2) %>% 
  arrange(padj)  

# Display the most significant DEGs
head(significant_degs, n = 10)  # Change n to show more or fewer DEGs
...
```{r}
# Extract significant gene IDs for each tissue type
lesion_genes <- rownames(res)[which(res$padj < 0.05 & res$log2FoldChange > 1 & res$tissue == "Lesion_treatment")]
nonlesion_genes <- rownames(res)[which(res$padj < 0.05 & res$log2FoldChange > 1 & res$tissue == "Nonlesion_treatment")]
healthy_genes <- rownames(res)[which(res$padj < 0.05 & res$log2FoldChange > 1 & res$tissue == "Healthy_treatment")]

# Create a Venn Diagram comparing DEGs from different tissue types
venn.plot <- draw.triple.venn(
  area1 = length(lesion_genes),
  area2 = length(nonlesion_genes),
  area3 = length(healthy_genes),
  n12 = length(intersect(lesion_genes, nonlesion_genes)),
  n23 = length(intersect(nonlesion_genes, healthy_genes)),
  n13 = length(intersect(lesion_genes, healthy_genes)),
  n123 = length(intersect(lesion_genes, nonlesion_genes, healthy_genes)),
  category = c("Lesion_treatment", "Nonlesion_treatment", "Healthy_treatment"),
